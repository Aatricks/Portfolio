...
proof: pulled by 70+ companies via Maven Central · powers Emaki summaries
...
It's published on Maven Central as `io.github.aatricks:llmedge`, where Sonatype's publisher insights show it being pulled by 70+ distinct companies each quarter. [`Emaki`](/Portfolio/work/emaki) ships llmedge in production for on-device chapter summaries, so the user's text never leaves the phone. The [`llmedge-examples`](https://github.com/Aatricks/llmedge-examples) repo has each feature as its own small Android app.
